...
class _CorrelationHeatmapState extends State<CorrelationHeatmap> {
  ...
  final _labels = const ['Yield', 'NDVI', 'Rain', 'TempMax', 'TempMin', 'Soil'];
  ...
  Color _cellColor(double value) {
    if (value > 0) {
      return Color.lerp(Colors.white, AppColors.limeGreen, value)!;
    }
    return Color.lerp(Colors.white, AppColors.burntOrange, -value)!;
  }

  String get _selectedLabel {
    if (_hoveredRow == null || _hoveredCol == null) return '';
    final value = _matrix[_hoveredRow!][_hoveredCol!].toStringAsFixed(2);
    return '${_labels[_hoveredRow!]} <-> ${_labels[_hoveredCol!]}: $value';
  }
                      ...
                      (row) => Expanded(
                        child: Row(
                          children: List.generate(_matrix[row].length, (col) {
                            final value = _matrix[row][col];
                            final isHovered =
                                _hoveredRow == row && _hoveredCol == col;
                            return Expanded(
